...
for TARGET in kubernetes admin kubelet
do
...
cat > $SSL_PATH/known_tokens.csv <<EOF
...
kubectl config set-credentials kubelet \
  --client-certificate=$SSL_PATH/kubelet.pem \
  --client-key=$SSL_PATH/kubelet-key.pem \
  --embed-certs=true \
  --kubeconfig=$KUBECONFIG_PATH/kubelet.kubeconfig
...
kubectl config use-context default --kubeconfig=$KUBECONFIG_PATH/kubelet.kubeconfig
